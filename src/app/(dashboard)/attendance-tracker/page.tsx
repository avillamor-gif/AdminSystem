...
import { REQUEST_TYPE_LABELS, DAY_TYPE_LABELS, OT_RATE_MULTIPLIERS, computeHours } from '@/services'
...
const OT_STATUS_COLORS: Record<OvertimeRequestStatus, string> = {
...
function OtObTab({ employeeId }: { employeeId: string }) {
  const { data: myRequests = [], isLoading } = useMyOvertimeRequests(employeeId)
  ...
  const handleSubmit = async () => {
    if (!employeeId || !requestDate || !startTime || !endTime || !purpose) {
...
export default function TimePage() {
  ...
  const [activeTab, setActiveTab] = useState<'punch' | 'timesheets' | 'attendance' | 'ot-ob'>('punch')
  ...
  return (
    <div className="space-y-6">
      ...
      {/* OT / OB Requests Tab */}
      {activeTab === 'ot-ob' && <OtObTab employeeId={currentEmployee?.id ?? ''} />}
